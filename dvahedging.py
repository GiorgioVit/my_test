...
from envs.output import compute_output, compute_output_step, write_output_df
...
class DVAHedging(Env):
    ...
    def __init__(self, market_datafile, horizon=100, window_offset=3, window_offset_day=0, risk_factor=1,
                 window_offset_week=0, gamma=1.0, dva_nominal=400e6, start_point=None,
                 Initial_alloc_tot=None, empty_allocation=1, training=True, verbose=0, low_bound=1, sep=','):
    ...
    def step(self, action):
        ...
        rate = self.data.ix[index, "CDS_Spread_1Y"] / 100
        r1 = 0.001  # SCELTO ARBITRARIAMENTE PER TEST, SARà DA MODIFICARE
        r0 = r1 + rate

        if chiusura:
            # calcolo variazione temporale -------------------------------------------
            current_day = datetime.strptime(self.data.ix[index, 'data_column'], '%d/%m/%Y')
            next_day = datetime.strptime(self.data.ix[index + 1, 'data_column'], '%d/%m/%Y')
        ...
        self.delta_itraxx = (self.data.ix[index, "Itraxx_onPrice_BID"] + self.data.ix[
            index, "Itraxx_onPrice_ASK"]) / 2 - (self.data.ix[index - 1, "Itraxx_onPrice_BID"] + self.data.ix[
            index - 1, "Itraxx_onPrice_ASK"]) / 2
        if self.data.ix[index, "ITRAXX_Time_To_Roll"] == 0:
            itraxx_dividend = self.data.ix[index, "ITRAXX_Dividend"] * chiusura
        else:
            itraxx_dividend = 0
        ...
        # calcolo del reward
        if chiusura:
            self.total_PL = step_gain + delta_DVA + dva_dividend
            reward = self.f_reward(self.total_PL)
            # reward = step_gain + delta_DVA + dva_dividend
        else:
            self.total_PL = step_gain + delta_DVA
            reward = self.f_reward(self.total_PL)
            # reward = step_gain + delta_DVA
        ...
        self.pl_cumulato += self.total_PL
        if chiusura:
        ...
        if self.print_output:
            ...
            print("\nP&L = dailygain + delta DVA: ", self.total_PL)  # corretto "-" con "+"
        ...
        if state is None:
            if self.start_point is not None:
                ...
                if self.training:
                    ...
                    u_bound = self.low_bound + round(3 / 4 * rows)
                else:
                    l_bound = self.low_bound + round(3 / 4 * rows) + 1
    ...
    def get_state(self):
        ...
        portfolio_sensitivities = [sensitivities[0]*allocation_tot[0], sensitivities[1]*allocation_tot[1],
                                   sensitivities[3]*allocation_tot[3], sensitivities[4]]
        return np.concatenate((np.array([allocation_tot[0],allocation_tot[1],allocation_tot[3]]), portfolio_sensitivities))

    """
    se empty_allocation=0 genera un portafoglio randomico non vuoto
    """
    ...
    def _compute_sensitivity(self, index):
        ...
        itraxx_sensitivity = cdata['Itraxx_Sensitivity_BID'] * self.multiplier[2] / 10000  # *self.allocation_tot[3]
    ...
    def _compute_RC(self, price, sensi, init=0):

        allocation_tot = self.allocation_tot
        multiplier = self.multiplier
        sensitivities_RC = np.zeros(4)
        sensitivities_RC[0] = allocation_tot[0] * sensi[-5] * multiplier[0]  # da percent sensitivity a sensitivity unitaria
        sensitivities_RC[1] = allocation_tot[1] * sensi[-4] * 10000          # da bp sensitivity a sensitivity unitaria
        sensitivities_RC[2] = allocation_tot[2] * sensi[-3] * 10000          # da bp sensitivity a sensitivity unitaria
        sensitivities_RC[3] = allocation_tot[3] * sensi[-2] * multiplier[2]  # da bp sensitivity a sensitivity unitaria
        ...
        WS_bund = RW[2] * sensitivities_RC[2]  # - because it's a short position
        WS_iTraxx = RW[3] * sensitivities_RC[3]  # - because it's a short position
    ...
    def _compute_dva_dividend(self, index):
        ...
        delta_days_CDS = datetime.strptime(self.data.ix[index - 1, 'maturity ISPIM'], '%d/%m/%Y') - datetime.strptime(
            self.data.ix[index, 'data_column'], '%d/%m/%Y')
        return 0.6 * (1 - np.exp(-lambda_CDS * delta_days_CDS.days / 360)) * self.dva_nominal - self._compute_dva(self,
                                                                                                                  index - 1)
    ...
    def _compute_delta_iTraxx(self, index):
        ...
        if allocation_delta[-2] > 0:
            delta_iTraxx = tmp_alloc_tot[2] * multiplier[2] * (iTraxx - iTraxx_old) \
                           + allocation_delta[-2] * multiplier[2] * (
            iTraxx - self.data.ix[index - 1, "Itraxx_onPrice_BID"])
        else:
            delta_iTraxx = + tmp_alloc_tot[2] * multiplier[2] * (iTraxx - iTraxx_old) \
                           + allocation_delta[-2] * multiplier[2] * (
            iTraxx - self.data.ix[index - 1, "Itraxx_onPrice_ASK"])
    ...
    def baseline_action(self, flag="btp"):
        ...
        if flag == "btp-iTraxx":
            ...
            iTraxx = round(-0.5 * sensitivity_dva / sensitivity_iTraxx)  # long risk
    ...
    """
    Metodo per ottenere le grandezze da salvare in output (se chiamato da init())
    """
    _compute_output = compute_output

    """
    Metodo per ottenere le grandezze da salvare in output (se chiamato da step())
    """
    _compute_output_step = compute_output_step

    """
    Metodo per salvare gli output su un foglio excel
    """
    _write_output_df = write_output_df
